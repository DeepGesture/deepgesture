...
# sampleCount = number of samples in the file
# featureCount = number of features per sample
def ReadDataText(textFile, sampleCount, featureCount):
    # bytesPerLine = featureCount * 4
    # data = []
    # with open(textFile, "r") as f:
    #     for i in np.arange(sampleCount):
    #         if i % max(int(sampleCount / VERBOSE_STEP), 1) == 0:
    #             print('Reading binary ' + textFile + '...', round(100 * i / sampleCount, 2), "%", end="\r")
    #         f.seek(i * bytesPerLine)
    #         bytes = f.read(bytesPerLine)
    #         data.append(np.float32(array.array('f', bytes)))
    # print('Reading binary ' + textFile + '...', 100, "%", end="\r")
    # print("")
    # return np.concatenate(data).reshape(sampleCount, -1)
    data = []
    with open(textFile, "r") as f:
        for i in np.arange(sampleCount):
            if i % max(int(sampleCount / VERBOSE_STEP), 1) == 0:
                print('Reading text ' + textFile + '...', round(100 * i / sampleCount, 2), "%", end="\r")
            line = f.readline()
            if not line:
                break
            features = np.fromstring(line.strip(), sep=' ', dtype=np.float32)
            if len(features) != featureCount:
                raise ValueError(f"Expected {featureCount} features, but got {len(features)}.")
            data.append(features)
    print('Reading text ' + textFile + '...', 100, "%", end="\r")
    print("")
    return np.concatenate(data).reshape(sampleCount, -1)
...
#     return ToDevice(torch.from_numpy(data))
    ...
    # features = 2904
    # batch = ReadBatch("Input.bin", np.random.randint(samples, size=batchSize), features)
def ReadBatchFromFile(textFile, sampleIndices, featureCount):
    data = np.empty([len(sampleIndices), featureCount], dtype=np.float32)
    ...
    with open(textFile, "r") as f:
        ...
        for i in range(len(sampleIndices)):
            # Select the line based on the sample index
            line = lines[sampleIndices[i]].strip()  # Get the line and strip any extra whitespace
            values = list(map(float, line.split()))  # Convert the space-separated values to floats

            # Ensure we have the right number of features
            if len(values) == featureCount:
                data[i] = values
            else:
                raise ValueError(f"Line {sampleIndices[i]} does not contain the expected {featureCount} features")
